...
* **Znění právních předpisů:** Texty zákonů a vyhlášek se do repozitáře **nepřepisují ručně**. Stahuje je `npm run sync:laws` z veřejného REST API e-Sbírky (https://e-sbirka.gov.cz/restful-api) do `public/data/esbirka/*.json` a `src/data/esbirka/snapshotManifest.ts` — oba jsou generované, needituj je. Pole `fullLegalText` v `vscrRegulationsRegistry.ts` je **studijní výběr ustanovení**, ne úplné znění; tak se také musí v rozhraní jmenovat. Nikdy nepiš do rozhraní „doslovné znění“, „úřední znění“ ani „ověřeno podle e-Sbírky“ nad textem, který z e-Sbírky nepochází nebo se proti ní právě neověřil — přesně tím byla vadná dřívější verze Právního kompasu.
* **Tvrzení o paragrafech:** Každý odkaz na paragraf v `legalCompasData.ts` a v `keyProvisions` musí existovat v platném znění; `npm run check:legal` to porovnává s osnovou z e-Sbírky a neexistující paragraf shodí build. Kontrola zároveň hlásí, jak doslovné jsou texty vydávané za znění zákona — než něco označíš za citaci, ověř si to proti staženému znění, ne po paměti.
