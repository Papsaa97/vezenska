...
| 39 | `038_zarazovani_do_trid.sql` | **Nová funkce.** Zařazování do tříd: žádost studenta, „Nevidím zde svou třídu“ s povinnou poznámkou, nominace velitelem s potvrzením označeného, přiřazení a jmenování velitele lektorem/správcem, dočasný zástupce velitele a předání funkce s odůvodněním (historie pro lektory). `user_class` smí přímo měnit jen správce; plnou nástěnku (`class_boards`) čtou jen členové třídy, lektoři a správci, ostatní dostanou `trida_prehled()`. Idempotentní |
| 40 | `039_diskuze_tridy.sql` | **Nová funkce.** Diskuze třídy na rozšířené nástěnce: příspěvky, ankety (2–10 možností, hlas lze měnit do konce) a označení členů třídy s oznámením do zvonku. Číst a psát smějí jen členové třídy, lektoři a správci; velitel, platný zástupce, lektor a správce připínají, skrývají a mažou. Tabulky jsou klientům zavřené, vše jde přes funkce. Spustit až po 038. Idempotentní |
| 41 | `040_postup_na_serveru_a_novy_obsah.sql` | **Oprava + nové funkce.** Tabulka `studijni_postup`: splněné scénáře a drily se ukládají k účtu, ne jen do prohlížeče (záložka Modelové situace ukazovala 0 / 12 na jiném zařízení nebo v PWA). Rozšiřuje `content_blocks_kind_check` o `weapon`, `stoppage_drill` a `jidelnicek`, aby lektor mohl upravovat záložku Zbraně a střelba a jídelníček na nástěnce. Idempotentní |
| 42 | `041_oblibene_predpisy_oznameni_velitele.sql` | **Oprava + nové funkce.** Spouští se po 040. `studijni_postup` přijímá oblíbené otázky a předpisy (ukládají se k účtu). `content_blocks_kind_check` dostává `regulation` (úpravy právních předpisů od lektora vidí všichni, dřív jen jeho prohlížeč). Adresát si smí smazat vlastní oznámení. Žádost o třídu bez velitele dostane zástupce, případně lektoři a správci. Velitel přeřazený z třídy nebo ve smazané třídě přestává být velitelem. Idempotentní |
